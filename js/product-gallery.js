...
const clickEvent = (elms, activeClass = "is-active", functor = fnX) => (fn) => {
  ...
  const meta = {
    activeClass,
    index: 0,
    event: new CustomEvent("laugh"),
    lastIndex: 0,
  };

  const getLastActive = (next) => (c, i) => {
    if (c.classList.contains(activeClass)) meta.lastIndex = i;
...
const showMenu = () => {
  ...
  const toggleMenu = () => {
    ...
    if (!$(".fancy-menu li.active")) navigations[0].click();
  };

  [...buttons].map((e) => e.addEventListener("click", toggleMenu));
  buttons[0].click();

  clickEvent(navigations, "active")((current, { index }) => slideNow(index));
};
